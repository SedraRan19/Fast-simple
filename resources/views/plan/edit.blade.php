...
@section('content')
    ...
    <form method="POST" action="{{route('update_plan',['id'=>$plan->id])}}">
        @csrf
        ...
        <div class="settings-panel">
            ...
            <div class="">
                ...
                <div class="px-4 py-2 row">
                    <input class="wrapper border-none background-white outline-none" name="name" placeholder="Enter plan name" value="{{$plan->name}}"/>
                    <div class="stroke-line wrapper pb-3"></div>
                </div>
                ...
                <div class="px-4 py-2 row">
                    <input type="number" class="wrapper border-none background-white outline-none" name="price" placeholder="Enter Plan Price" value="{{$plan->price}}" />
                    <div class="stroke-line wrapper pb-3"></div>
                </div>
                ...
                <div class="px-4 py-2 row">
                    <select class="wrapper border-none background-white outline-none p-2" name="frequency">
                        ...
                        <option value="week" {{ old('frequency', $plan->frequency) == 'week' ? 'selected' : '' }}>Week</option>
                        <option value="month" {{ old('frequency', $plan->frequency) == 'month' ? 'selected' : '' }}>Month</option>
                        <option value="year" {{ old('frequency', $plan->frequency) == 'year' ? 'selected' : '' }}>Year</option>
                        <option value="day" {{ old('frequency', $plan->frequency) == 'day' ? 'selected' : '' }}>Day</option>
                    </select>
                ...
                <div class="px-4 py-2 row">
                    <input type="number" class="wrapper border-none background-white outline-none" name="no_of_users_included" placeholder="Enter Number Of Users Included" value="{{$plan->no_of_users_included}}"/>
                    <div class="stroke-line wrapper pb-3"></div>
                </div>
                ...
                <div class="px-4 py-2 row">
                    <input class=" wrapper border-none background-white outline-none" name="price_per_additional_user" placeholder="Enter Price Per Additional User" value="{{$plan->price_per_additional_user}}"/>
                    <div class="stroke-line wrapper pb-3"></div>
                </div>

                <div class="row p-2 back-white">
                    <div class="col-md-12">
                        <textarea rows="10"  class="wrapper border-none background-white outline-none" spellcheck="true" name="description" placeholder="Type Description">{{$plan->description}}</textarea>
                        <div class="stroke-line wrapper pb-3"></div>
                    </div>
                </div>
                ...
                @php
                    $permissionPlan = App\Models\Plan_permission::where('plan_id',$plan->id)->get()->pluck('permission_id');    
                @endphp

                @foreach($permissions as $permission)
                <div class="px-4 py-2 row">
                    <input type="checkbox" name="permission[]" value="{{$permission->id}}" class="roles_checkbox" {{ $permissionPlan->contains($permission->id) ? 'checked' : '' }} /> 
